...
const Login = () => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen bg-black-gradient py-6 sm:px-6 lg:px-8`}>
        ...
        <div className="w-full max-w-md px-8 pt-6 pb-8 mb-4 bg-black-gradient rounded-[20px] shadow-md feature-card">
            ...
            <form onSubmit={funcAutenticacion}>
                ...
                <div className='flex items-center justify-between'>
                   <button className={`py-4 px-6 font-poppins font-medium text-[18px] text-primary bg-gold-gradient rounded-[20px] outline-none ${styles}`}>{registrando ? 'Registrate' : 'Inicia Sesion'}</button>
                </div>

            </form>
            <h2 className='font-poppins mb-4 text-xl font-semibold text-gradient m-4'>{registrando ? 'Si ya tienes cuenta:  ' : 'No tienes cuenta:  '}<button className='font-poppins font-medium text-[22px] rounded-[10px] outline-none' onClick={()=>setRegistrando(!registrando)}>{registrando ? ' Inicia sesion' : ' Registrate'}</button> </h2>
        </div>
    </div>
  );
};
